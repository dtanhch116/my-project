            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="box">
                    <div class="box__input">
                        <input type="hidden" value="<?=$show_one['id_sp']?>">
                        <label for="">Tên Sản Phẩm</label>
                        <?php if(isset($err['ten'])):?>
                            <span style="color: red;"><?= $err['ten'] ?></span>
                            <?php endif?>
                        <input type="text" class="input" name="ten_sp" value="<?=$show_one['ten_sp']?>">

                        <label for="">Đơn Giá</label>
                        <?php if(!empty($err['don_gia'])):?>
                            <span style="color: red;"><?= $err['don_gia'] ?></span>
                            <?php endif?>
                        <input type="text" class="input" name="don_gia" value="<?=$show_one['don_gia']?>">

                        <label for="">Giảm giá</label>
                        <?php if(!empty($err['giam_gia'])):?>
                            <span style="color: red;"><?= $err['giam_gia'] ?></span>
                            <?php endif?>
                        <input type="text" class="input" name="giam_gia" value="<?=$show_one['giam_gia']?>">

                        <label for="">Ảnh Sản Phẩm</label>
                        <?php if(!empty($err['anh'])):?>
                            <span style="color: red;"><?= $err['anh'] ?></span>
                            <?php endif?>
                            <img src="../../project1/public/layout/images/product/<?=$show_one['anh_sp']?>" alt="" width="100px" height="100px">
                            <input type="hidden" name="anh_sp_one" value="<?= $show_one['anh_sp']?>">
                    ...
                    <div class="box__input">
                        ...
                        <select name="dac_biet" class="input" id="">
                            <option value="Đặc Biệt"<?= ($show_one['dac_biet'] == "Đặc Biệt") ? 'selected' : ''?>>Đặc Biệt</option>
                            <option value="Bình Thường"<?= ($show_one['dac_biet'] == "Bình Thường") ? 'selected' : ''?>>Bình Thường</option>
                        </select>
                        ...
                        <?php if(!empty($err['ngay_nhap'])):?>
                            ...
                            <?php endif?>
                        <input type="date" class="input" name="ngay_nhap" value="<?=$show_one['ngay_nhap']?>">
                ...
                <?php if(!empty($err['mo_ta'])):?>
                            ...
                            <?php endif?>
                <textarea name="mo_ta" class="mota textarea" cols="30" rows="10"><?=$show_one['mo_ta']?></textarea>



                <button class="t9d0ea btn_t9d0ea2 t9d0ea1" type="submit" name="btn_sanpham">Cập Nhật</button>
            </form>
        </div>
    </div>
</body>
